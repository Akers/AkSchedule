...
public class DelDilog extends JFrame
{
	public DelDilog(MainGuiFrame mgf)
	{
		super();
		this.dp = mgf;
		...
		BTN_del.addActionListener(new DelAction());
		//取消按钮
		...
		//输入框所在面板
		Border input = BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "即将删除的日程");
		panel_input.add(TXT_sch);
	...
	public void deleteit(String week, int index)
	{
		dr.deleSchedule(week, index);
		dr.saveData();
		TXT_sch.setText("");
		dp.resetPanel();
		dp.repaint();
	...
	private class DelAction implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				int res = JOptionPane.showConfirmDialog(null,"是否确认要删除日程“"+dr.getData(week, index)+"”？","删除确认",JOptionPane.YES_NO_OPTION);
